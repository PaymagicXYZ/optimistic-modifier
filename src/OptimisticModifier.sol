...
contract OptimisticModifier is Modifier {
    event DelaySetup(
    ...
    /// @param _executionFee Fee in wei added to executions done through executeNextTxReimburse()
    ...
    function executeNextTxReimburse(
        address to,
        ...
        uint256 reimbursementWithFee = ((startGas - gasleft() + 39706) *
            tx.gasprice) + executionFee;
        require(
            avatar.balance >= reimbursementWithFee,
            "Not enough ether in safe for reimbursement with fee"
        );
        require(
            exec(
                refundReceiver,
                reimbursementWithFee,
                "0x",
                Enum.Operation.Call
            )
        );
    ...
    /// @dev Checks whether msg.sender is an owner of the avtar
    modifier onlySafeOwner() {
        (bool success, bytes memory result) = avatar.staticcall(
            abi.encodeWithSignature("isOwner(address)", msg.sender)
        );
        success = abi.decode(result, (bool));
        require(success, "Msg.sender is not a safe owner");
        _;
    }
    ...
    /// @notice This can only be called by the owner
    function setTxNonce(uint256 _nonce) public onlySafeOwner {
        require(
            _nonce > txNonce,
    ...
    function setExecutionFee(uint256 fee) public onlySafeOwner {
        executionFee = fee;
    }
}
